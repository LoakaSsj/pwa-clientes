...
def create_app():
    ...
    # Configuración DB
    DB_HOST = os.environ.get("DB_HOST", "185.232.14.52")
    DB_USER = os.environ.get("DB_USER", "u760464709_21005356_usr")
    DB_PASSWORD = os.environ.get("DB_PASSWORD", ";h5U3KOP")
    DB_NAME = os.environ.get("DB_NAME", "u760464709_21005356_bd")

    # --- Pool de conexiones ---
    db_pool = pooling.MySQLConnectionPool(
        pool_name="clientes_pool",
        pool_size=5,
        host=DB_HOST,
        user=DB_USER,
    ...
    @login_required
    def clientes_page():
        response = make_response(render_template("clientes.html"))
        response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
        response.headers["Pragma"] = "no-cache"
        response.headers["Expires"] = "0"
        return response
    ...
    def create_cliente():
        ...
        try:
            ...
            return jsonify(
                {"success": True, "data": {"id": new_id, "nombre": nombre, "saldo": saldo_val}}
            )
        except Error as e:
            return jsonify({"success": False, "error": str(e)}), 500
        finally:
    ...
    def update_cliente(cliente_id):
        ...
        try:
            ...
            return jsonify(
                {"success": True, "data": {"id": cliente_id, "nombre": nombre, "saldo": saldo_val}}
            )
        except Error as e:
            return jsonify({"success": False, "error": str(e)}), 500
        finally:
